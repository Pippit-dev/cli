...
通过 小云雀的API 创建会话、发送消息（生图、生视频、编辑视频等）、上传图片/视频/音频文件，并查询会话消息进展。
...
3. **上传文件** - 支持上传`单张图片`、`单个视频文件`或`单个音频文件`到小云雀资产库，得到文件对应的 `asset_id`（编辑或参考已有图片/视频/音频时需要先上传）
...
- 当用户提供了参考的文件地址时，先进行文件上传，仅支持图片、视频、音频。
...
- 上传文件仅支持图片（image/*）、视频（video/*）和音频（audio/*）类型，其他类型会被拒绝，文件大小须在 200MB 以下
