...
API 错误响应中，只有与请求 Run ID 匹配的结构化状态明确为失败或取消，CLI 才返回 `completed=true`、Run 失败原因及可用 LogID。错误响应缺少匹配的终态时，返回 `completed=false` 和以“查询失败：”开头的错误，保留 LogID 与任务 ID。此时未确认 Run 状态；排障后可查询同一 Run，不从错误文案推断生成结果，也不自动重新生成或无限轮询。
